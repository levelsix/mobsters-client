...
@property (nonatomic, retain) NSMutableArray *queuedFrames;
...
- (id)initWithHost:(NSString *)aHost
  ...
  if(self = [super init]) {
    ...
    self.queuedFrames = [[NSMutableArray alloc] init];
	}
	return self;
}
...
- (void)sendFrameWithCommand:(NSString *)command
  ...
  if (self.socket.readyState != SR_OPEN) {
    [self.queuedFrames addObject:frame];
  }
  LogDebug(@">>> %@", frame);
...
- (void)sendPing:(NSTimer *)timer  {
  ...
  NSData *lfData = [NSData dataWithBytes:"\x0A" length:1];
  [self.socket send:lfData];
...
- (void) webSocketDidOpen:(SRWebSocket *)webSocket {
  for (STOMPFrame *frame in self.queuedFrames) {
    [self sendFrameWithCommand:frame.command headers:frame.headers body:frame.body];
  }
}
...
- (void) webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
  ...
  NSError *err = [NSError errorWithDomain:reason code:code userInfo:@{@"was_clean":@(wasClean)}];
  if (!self.connected && self.connectionCompletionHandler) {
