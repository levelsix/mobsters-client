...
- (void) spawnClanBattleSpriteForClanMemberAttack:(ClanMemberAttack *)cma comeFromTop:(BOOL)comeFromTop {
  ...
  BattleSprite *clanBs = [[BattleSprite alloc] initWithPrefix:mp.imagePrefix nameString:mp.displayName rarity:mp.quality animationType:mp.attackAnimationType isMySprite:NO verticalOffset:mp.verticalPixelOffset];
...
- (void) clanMemberAttacked {
  if (self.clanMemberAttacks.count > 0) {
    ...
    [self dealDamage:cma.attackDamage enemyIsAttacker:NO withSelector:@selector(checkEnemyHealth)];
