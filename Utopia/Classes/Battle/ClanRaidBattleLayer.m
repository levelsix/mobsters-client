...
- (id) initWithEvent:(PersistentClanEventClanInfoProto *)event myUserMonsters:(NSArray *)monsters puzzleIsOnLeft:(BOOL)puzzleIsOnLeft {
  if ((self = [super initWithMyUserMonsters:monsters puzzleIsOnLeft:puzzleIsOnLeft gridSize:CGSizeMake(9, 9)])) {
...
- (void) spawnClanBattleSpriteForClanMemberAttack:(ClanMemberAttack *)cma comeFromTop:(BOOL)comeFromTop {
  ...
  BattleSprite *clanBs = [[BattleSprite alloc] initWithPrefix:mp.imagePrefix nameString:[[NSAttributedString alloc] initWithString:mp.displayName] rarity:mp.quality element:mp.monsterElement animationType:mp.attackAnimationType isMySprite:NO verticalOffset:mp.verticalPixelOffset];
...
- (void) clanMemberAttacked {
  if (self.clanMemberAttacks.count > 0) {
    ...
    [self dealDamage:cma.attackDamage enemyIsAttacker:NO usingAbility:NO withTarget:self withSelector:@selector(checkEnemyHealthAndStartNewTurn)];
