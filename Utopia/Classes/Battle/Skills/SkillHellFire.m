...
- (BOOL) updateBulletOrbs
{
  ...
  for (NSInteger column = 0; column < layout.numColumns; ++column)
  {
    for (NSInteger row = 0; row < layout.numRows; ++row)
    {
      ...
      if (orb.specialOrbType == SpecialOrbTypeBullet && orb.bulletCounter > 0)
      {
        // Update counter
        --orb.bulletCounter;
        ...
        if (orb.bulletCounter <= 0) // Use up the bullet orb
        {
          ...
          [sprite updateBulletCounter:YES];
      }
    }
  }
...
- (void) spawnBulletOrbs:(NSInteger)count withTarget:(id)target andSelector:(SEL)selector
{
  ...
  for (NSInteger n = 0; n < count; ++n)
  {
    ...
    orb.bulletCounter = _orbsSpawnCounter;
