    ...
    for (BattleOrb *orb in powerupChain.orbs) {
      if (orb.powerupType && ![usedPowerupOrbs containsObject:orb]) {
        ...
        if (orb.powerupType == PowerupTypeAllOfOneColor) {
          orb.orbColor = powerupChain.powerupInitiatorOrb.orbColor;
        }
...
- (NSArray *)topUpOrbs {
  ...
  for (NSInteger column = 0; column < _numColumns; column++) {
    ...
    for (NSInteger row = _numRows - 1; row >= 0 && _orbs[column][row] == nil; row--) {
      ...
      if (_tiles[column][row] != nil) {
        ...
        do {
          newOrbColor = arc4random_uniform(_numColors) + OrbColorFire;
        ...
        [array insertObject:orb atIndex:0];
      }
    }
  }
  return columns;
