...
- (void) update {
  ...
  if (gs.connected) {
    ...
    if (silver != _curSilver) {
      ...
      self.topBarView.silverLabel.text = [@"$" stringByAppendingString:[Globals commafyNumber:_curSilver+change]];
      _curSilver += change;
    }
    ...
    if (gold != _curGold) {
      ...
      self.topBarView.goldLabel.text = [Globals commafyNumber:_curGold+change];
      _curGold += change;
    }
    ...
    if (gs.currentEnergy != _curEnergy) {
      ...
      [self.topBarView.energyBar setPercentage:(_curEnergy+change)/((float)gs.maxEnergy)];
      _curEnergy += change;
      self.topBarView.energyLabel.text = [NSString stringWithFormat:@"%d/%d", _curEnergy, gs.maxEnergy];
    }
    ...
    [self.topBarView.expBar setPercentage:(_curExp+change-gs.expRequiredForCurrentLevel)/(float)(gs.expRequiredForNextLevel-gs.expRequiredForCurrentLevel)];
    self.topBarView.expLabel.text = [NSString stringWithFormat:@"%d/%d", _curExp-gs.expRequiredForCurrentLevel, gs.expRequiredForNextLevel-gs.expRequiredForCurrentLevel];
    _curExp += change;
